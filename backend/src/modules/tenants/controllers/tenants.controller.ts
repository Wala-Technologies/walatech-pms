...
export class TenantsController {
  ...
  @Public()
  @Get('admin/all')
  @ApiOperation({ summary: 'Get all tenants for admin interface (no auth required)' })
  @ApiResponse({ status: 200, description: 'All tenants retrieved successfully' })
  findAllForAdmin() {
    return this.tenantsService.findAll();
  }
  ...
  async getUserTenants(@CurrentUser() user: any) {
  ...
  @ApiResponse({ status: 200, description: 'Tenant settings reset successfully' })
  @ApiResponse({ status: 404, description: 'Tenant not found' })
  ...
  async uploadLogo(
    ...
    const baseUrl = process.env.NODE_ENV === 'production' ? `${protocol}://${host}` : `${protocol}://${host}:${port}`;
    const logoUrl = `${baseUrl}/api/tenants/${id}/logo/${file.filename}`;
    
    // Update tenant settings with new logo URL
    const currentSettings = await this.tenantSettingsService.getTenantSettings(id);
    const updatedSettings = {
      ...currentSettings,
      companyLogo: logoUrl,
    };
    
    await this.tenantSettingsService.updateTenantSettings(id, { settings: updatedSettings });
